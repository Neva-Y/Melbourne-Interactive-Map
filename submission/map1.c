...
/* Main binding function for map1 nearest point search
*/
int main(int argc, char **argv) {  
    ...
    coords_t *inputCoordinates = read_input_coordinates(&inputLines);
    ...
    /* Search and print out the node closest to the coordinate */
    kdnode_t *closestNode;
    data_t *closestNodeData;
    int count = 0;
    for (i = 0; i < inputLines; i++) {
        closestNode = search_KDtree(NULL, dataKDT->root, inputCoordinates[i],
                                    ROOTDEPTH, INFINITY, &count);
        closestNodeData = (data_t *)closestNode->head->data;
        print_node(closestNode, inputCoordinates[i], writeFile);
        printf("%.12g %.12g --> %d\n", inputCoordinates[i].x,
               inputCoordinates[i].y, count);
        count=0;
    }
