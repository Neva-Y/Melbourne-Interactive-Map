...
def ass2_part_one(search_point, zoom_on_found):
    ...
    program = subprocess.Popen(['bash', '-c', "cd submission && (echo {} > results/stdin) && rm -f *.o && make -B map1 && (./map1 clue.csv results/outfile < results/stdin)".format(" ".join([str(n) for n in search_point]))],
    ...
    bounds = [-37.85750715625204, 144.898681640625, -
              37.762029885732105, 145.008544921875]
...
def ass2_part_two(search_point, radius, zoom_on_found):
    program = subprocess.Popen(['bash', '-c', "cd submission && mkdir -p results && echo {} > results/stdin2 && rm -f *.o && make -B map2 && ./map2 clue.csv results/outfile2 < results/stdin2".format("{} {}".format(" ".join([str(n) for n in search_point]), radius))],
                               stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    timelog(" - stdout - ")
    print(program.stdout.read().decode())
    timelog(" - stderr - ")
    error_text = program.stderr.read().decode()
    timelog("Opening outfile2")
    with open("submission/results/outfile2") as f:
        file_contents = f.read()
    timelog("Outfile contents:")
    print(file_contents)
    timelog("Processing")
    ...
    df = pd.DataFrame(data={'latitude': lat, 'longitude': lon})
    fig, ax = plt.subplots(figsize=(25.59, 28.15))
    circles = [plt.Circle((search_point[0], search_point[1]),
                          radius=radius, linewidth=0, alpha=0.7)]
